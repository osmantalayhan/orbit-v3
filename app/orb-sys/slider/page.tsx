...
export default function AdminSliderPage() {
  ...
  const fetchSliders = async () => {
    try {
      setLoading(true);
      const res = await apiClient(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/slider`);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await apiClient(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/slider/${id}`, {
        method: "DELETE"
      });
  ...
  const handleToggleActive = async (item: SliderItem) => {
    try {
      ...
      const res = await apiClient(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/slider/${item.id}`, {
        method: "PUT",
        body: fd
      });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId 
        ? `${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/slider/${editingId}`
        : `${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/slider`;
                      ...
                      <td style={{ paddingLeft: '24px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                          <img 
                            ...
                            style={{ width: '60px', height: '40px', objectFit: 'cover', borderRadius: '4px', backgroundColor: '#1a1a1a' }} 
                            onError={(e) => {
