...
export default function SettingsAdminPage() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const res = await apiClient(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/users?t=${new Date().getTime()}`, { cache: "no-store" });
  ...
  const handleAddUser = async () => {
    ...
    try {
      const res = await apiClient(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/users`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const res = await apiClient(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/users/${showPasswordModal}/password`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteUser = async (id: number) => {
    ...
    try {
      const res = await apiClient(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/users/${id}`, {
        method: "DELETE",
      });
      const data = await res.json();
