...
export default function AdminSalesChannelsPage() {
  ...
  const fetchChannels = async () => {
    try {
      setLoading(true);
      const res = await apiClient(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/sales-channels`);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await apiClient(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/sales-channels/${id}`, {
        method: "DELETE"
      });
  ...
  const handleToggleActive = async (item: SalesChannel) => {
    try {
      ...
      const res = await apiClient(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/sales-channels/${item.id}`, {
        method: "PUT",
        body: fd
      });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId 
        ? `${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/sales-channels/${editingId}`
        : `${process.env.NEXT_PUBLIC_API_URL}/api/v1/orb-sys/sales-channels`;
