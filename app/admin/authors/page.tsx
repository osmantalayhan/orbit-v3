...
export default function AuthorsAdminPage() {
  ...
  const fetchAuthors = async () => {
    try {
      const res = await apiClient(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/admin/authors`, { cache: "no-store" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingId 
      ? `${process.env.NEXT_PUBLIC_API_URL}/api/v1/admin/authors/${editingId}`
      : `${process.env.NEXT_PUBLIC_API_URL}/api/v1/admin/authors`;
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await apiClient(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/admin/authors/${id}`, {
        method: "DELETE"
      });
